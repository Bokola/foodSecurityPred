...
def clean_scientific_brackets(df):
    df = df.copy()
    # Aggressive regex: keep only alphanumeric and underscores for headers
    ...
    for col in df.columns:
        ...
        # Force numeric: remove brackets, then coerce to float
...
def run_hp_tuning():
    ...
    for cluster in ["p", "ap", "other"]:
        ...
        df_cluster = input_master[input_master["lhz"] == cluster] if "lhz" in input_master.columns else input_master.copy()
        ...
        for lead in [0, 1, 2, 3, 4, 8, 12]:
            ...
            # Force base_score to a float during tuning
            search = GridSearchCV(
                XGBRegressor(random_state=42, objective="reg:squarederror", base_score=0.5, tree_method="hist"),
                {"learning_rate": [0.01, 0.1], "max_depth": [4, 6], "n_estimators": [200]},
